...
type Challenge = { level: number; par: number; kind: 'journey' | 'daily' };

/**
 * The site is a static export, so the challenge is read on the client from the query string
 * the iMessage bubble carried: /c?lv=42&par=11&kind=journey.
 */
function readChallenge(): Challenge | null {
  ...
  const kind = q.get('kind') === 'daily' ? 'daily' : 'journey';
  return { level, par: Number.isFinite(par) && par > 0 ? par : 0, kind };
}
...
export function ChallengeLanding() {
  ...
  const title = challenge ? (challenge.kind === 'daily' ? "Today's Daily Brick" : `Level ${challenge.level}`) : 'An OutBrick challenge';
  ...
  const appUrl = challenge
    ? `outbrick://level/${challenge.level}?par=${challenge.par}&kind=${challenge.kind}`
    : 'outbrick://play';
    ...
    <div className="site-shell play-shell">
      ...
      <main className="play-main">
        <section className="play-intro">
          <div>
            ...
            <p>
              ...
              Match every brick to its gate, undo as often as you like, and beat their move count.
            </p>
          ...
          <div className="play-device-card">
            <div className="device-topline">
              ...
              <span>{ready && challenge ? (challenge.kind === 'daily' ? 'DAILY BRICK' : `LEVEL ${String(challenge.level).padStart(3, '0')}`) : 'OUTBRICK'}</span>
            </div>
        ...
        <section className="play-steps">
          <div><span className="play-step-number">01</span><h2>Get the game</h2><p>OutBrick is a free download on iPhone, iPad and Apple Watch. No ads, no lives, no timer.</p></div>
          <div><span className="play-step-number">02</span><h2>Open the board</h2><p>{ready && challenge ? `Head to ${challenge.kind === 'daily' ? 'the Daily Brick' : `Level ${challenge.level}`} and play it.` : 'Head to the board you were sent and play it.'}</p></div>
